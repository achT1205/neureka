<template>
  <v-flex>
    ...
    >
      ...
      >
        <v-card :loading="saving">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn class="ma-2" color="secondary" @click="cancel">Cancel</v-btn>
            <v-btn class="ma-2" color="primary" @click="save">Send</v-btn>
            <v-snackbar
              v-model="snackbar"
              :timeout="timeout"
              absolute
    ...
    <v-flex v-else class="full-flex-width">
      <v-alert class="full-flex-width text-center" color="warning" style="color: white">
        There is currently
        <strong class="ml-2 mr-2">Visit booked </strong> for you, or any
        <strong class="ml-2 mr-2">information </strong> to fillin
      </v-alert>
    </v-flex>
  </v-flex>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters(["clientVisit", "endEditing"]),
  },
  methods: {
